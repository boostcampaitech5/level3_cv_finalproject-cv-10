...
def on_predict_start(predictor):
    ...
    predictor.args.tracking_config = (
        ROOT
        / "boxmot"
        / predictor.args.tracking_method  # edited
        / "configs"
...
def run(args):
    ...
    for frame_idx, batch in enumerate(predictor.dataset):
        ...
        visualize = (
            increment_path(save_dir / Path(path[0]).stem, exist_ok=True, mkdir=True)
        ...
        for i in range(n):
            ...
            if predictor.args.save and predictor.plotted_img is not None:
                results = predictor.save_preds(
                    vid_cap, i, str(predictor.save_dir / p.name)
                )
                if predictor.args.streamlit:
...
def parse_opt():
    ...
    parser.add_argument(
        "--yolo-model",
        type=Path,
        default="/opt/ml/level3_cv_finalproject-cv-10/weights/yolov8n_100epoch_.pt",
    ...
    parser.add_argument(
        "--hide-label", action="store_true", help="hide labels when show"
    )
    parser.add_argument(
        "--hide-conf", action="store_true", help="hide confidences when show"
    )
    parser.add_argument(
